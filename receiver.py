...
class Receiver:
    ...
    def receive_frame(self, raw_bytes):
        """Obsługa odebranych danych (ciąg bajtów)."""
        ...
        # ### POPRAWKA 1: Musisz zdefiniować 'sn' zanim go użyjesz w printach!
        sn = frame.seq_num

        # 1. Sprawdzenie Błędu (korzysta z flagi ustawionej przy deserializacji)
        if frame.is_corrupt():
            # Nawet jeśli SN jest niepewny (bo ramka zepsuta), używamy go do logów
            print(
                f"{Colors.for_sn(sn)}[ODBIORNIK]: Otrzymano USZKODZONĄ ramkę DATA SN={sn} (Błąd CRC). ODRZUCAM.{Colors.RESET}")

            # ### POPRAWKA 2: NIE zwracaj None! Wyślij duplikat ACK!
            # To zapobiega zastojom przy modelu Gilberta.
            ...
            print(f"{self._ack_color_for_data_sn(ack_sn)}[ODBIORNIK]: Powtarzam ACK SN={ack_sn} "
                  f"(po błędzie CRC w DATA).{Colors.RESET}")
